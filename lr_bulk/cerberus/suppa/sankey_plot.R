...
transcripts <- read.table('1006/cerberus_transcript_novelty.tsv', sep = '\t', header = T, check.names = F, comment.char = '')
...
tot <- ggplot(df, aes(y = n, axis1 = lapa, axis2 = gtex,
  ...
  scale_x_discrete(limits = c('Lapa', 'GTEx'), expand = rep(0.1, 2)) +
  scale_fill_manual(values = cols) + labs(x = 'Source', y = 'Count', title = 'Transcripts between Lapa and GTEx') +
...
gtex_to_cerberus <- read.table('1006/gtex_to_cerberus_id.tsv', header = T)

AF <- sankeyPlot_event(transcripts, 
                       lapa_ioe_path = 'localEvents/cerberus.events_AF_strict.ioe', 
                       gtex_ioe_path ='localEvents_GTEx/flair.events_AF_strict.ioe', 
                       gtex_to_cerberus, 'AF')

AL <- sankeyPlot_event(transcripts, 
                       lapa_ioe_path = 'localEvents/cerberus.events_AL_strict.ioe', 
                       gtex_ioe_path ='localEvents_GTEx/flair.events_AL_strict.ioe', 
                       gtex_to_cerberus, 'AL')

A3 <- sankeyPlot_event(transcripts, 
                       lapa_ioe_path = 'localEvents/cerberus.events_A3_strict.ioe', 
                       gtex_ioe_path ='localEvents_GTEx/flair.events_A3_strict.ioe', 
                       gtex_to_cerberus, 'A3')


A5 <- sankeyPlot_event(transcripts, 
                       lapa_ioe_path = 'localEvents/cerberus.events_A5_strict.ioe', 
                       gtex_ioe_path ='localEvents_GTEx/flair.events_A5_strict.ioe', 
                       gtex_to_cerberus, 'A5')

MX <- sankeyPlot_event(transcripts, 
                       lapa_ioe_path = 'localEvents/cerberus.events_MX_strict.ioe', 
                       gtex_ioe_path ='localEvents_GTEx/flair.events_MX_strict.ioe', 
                       gtex_to_cerberus, 'MX')

RI <- sankeyPlot_event(transcripts, 
                       lapa_ioe_path = 'localEvents/cerberus.events_RI_strict.ioe', 
                       gtex_ioe_path ='localEvents_GTEx/flair.events_RI_strict.ioe', 
                       gtex_to_cerberus, 'RI')

SE <- sankeyPlot_event(transcripts, 
                       lapa_ioe_path = 'localEvents/cerberus.events_SE_strict.ioe', 
                       gtex_ioe_path ='localEvents_GTEx/flair.events_SE_strict.ioe', 
                       gtex_to_cerberus, 'SE')

library(patchwork)
p <- (AF + AL) / (A3 + A5) / (MX + RI + SE) + plot_layout(guides = 'collect')
ggsave('sankey_plot_per_event.pdf',p, width = 10, height = 10)
